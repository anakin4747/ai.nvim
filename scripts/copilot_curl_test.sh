...
curl_token() (
    curl \
        --request "GET" \
        --url "${GITHUB_URL}/copilot_internal/v2/token" \
...
curl_token_bad_bearer() (
    curl \
        --request "GET" \
        --url "${GITHUB_URL}/copilot_internal/v2/token" \
        --header "authorization: Bearer REDACTED_GITHUB_TOKEN" \
        --header "accept: application/json"
)
...
curl_chat() (
    ...
    curl \
        --request "POST" \
        --url "${COPILOT_URL}/chat/completions" \
        --header "authorization: Bearer $(get_session_token)" \
        --header "accept: application/json" \
        --header "content-type: application/json" \
        --header "copilot-integration-id: vscode-chat" \
...
PS3="curl who? "
select request in "localhost" "github"; do
    case $request in
        github)
